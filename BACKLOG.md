...
### Contact / Job-Change Intelligence — existing-customer continuity risk + prospect warm paths

**Classification: LATER / evidence-backed product hypothesis, not a committed feature.** Surfaced 2026-08-26 by a separate HA Sales Operator's firsthand sales pattern, independently reinforced by the adjacent warm-path hypothesis already raised while building Find More Like Them (see entry above) — reconciled against current code the same day, recon only, nothing implemented.

**Observed evidence:**
- Firsthand sales pattern: a known buyer/champion at an existing customer leaves the company; nobody notices; six months later the account goes quiet because the relationship left with that person.
- A separate Sales Operator independently surfaced this as a high-value missed signal.
- Find More Like Them's own build independently raised the adjacent hypothesis of detecting a former customer contact now working at a target company (warm-path prospecting).

**What already exists — do not duplicate:**
- **Contact identity persistence is real and sufficient to build on.** Contact Durability V1 (banked) gives every uploaded contact a durable `id`, an `origin` flag, and re-upload reconciliation (`dashboard/index.html`'s contact-matching logic) that preserves identity across CSV re-uploads and any manual edit. This is enough to track "is this the same person" over time — a second, independent detection channel (diff what a fresh upload's contact list contains against what was previously known) exists structurally today even before any new web research is added.
- **A person-centric search query already exists, but with a real, confirmed coverage gap.** `api/research-batch.js`'s `queryTemplates()` already issues `"${contactName}" "${company}" (appointed OR promoted OR ... OR "new company")` for existing-customer monitoring, and the synthesis prompt already carries explicit doctrine ("Leadership and contact-change rules": treat known-contact role changes and known-contact company changes as Leadership Change when person/company/role are credibly sourced). But `context.contactName` (`api/research-batch.js` line ~3518) resolves to a SINGLE name — `a.contactName || a.contacts[0]?.name` — so only the first uploaded contact per account is ever checked this way, not every known contact. And this mechanism can only catch a *publicly announced* move (a press mention, a "joins as" article) — it structurally cannot catch the far more common case of a quiet, unannounced departure, because it is a "search for evidence something happened" query, not a "verify a previously-established fact is still true" check. No code anywhere in the monitoring pipeline does the latter today — it is a genuinely different, absence-detecting pattern, not an extension of the existing event-driven search model.
- Recommended-buying-team/suggested-contact inference (`recommendedBuyingTeam`/`suggestedContactDetails`) already exists and is unrelated to this — that's about which role to approach for a given signal, not whether a specific known person changed jobs.

**Four candidate signal types — evaluated separately, not equal value or reliability:**
1. **Known contact left the existing customer** — the highest-value, most-requested case, and the LEAST supported today: requires the new absence-detection pattern above (neither the existing query model nor anything else in the codebase does this). Real relationship-continuity risk; should become a legitimate, timely Reason to Reach Out on the existing account, not a silent gap.
2. **Known contact changed role inside the existing customer** — partially supported today via the existing person+company query, limited to `contacts[0]`; extending to all known contacts is a bounded, well-scoped fix, not a new capability.
3. **New relevant leader joined the existing customer** — already the closest fit to the existing "Leadership Change" signal family and existing company-wide leadership queries; the gap is only that it isn't distinguished from the more relationship-critical cases above.
4. **Former known contact from Customer A now works at Prospect/Target B** — the warm-path case. Requires a genuinely new query shape (a person-plus-new-company search, not anchored to the OLD company at all) and materially higher identity-verification risk (common names, no way to confirm it's the same person without strong corroborating evidence). Rare but potentially the single highest-value signal type when it hits — matches the warm-path hypothesis already parked in Find More Like Them above. Confirms, rather than contradicts, that entry's existing economic-separation doctrine: this must not run across every generated lookalike candidate.

**Evidence/reliability — public sources only, never LinkedIn scraping:** company leadership/team pages, press releases and company announcements, public bios, conference/speaker/association pages, and search-result text that explicitly states prior or current employment. Person identity is materially more error-prone than the company-identity grounding `verifyCandidateCompanyGrounding()` already does — no equivalent person-identity grounding exists anywhere in the codebase today (the existing `potentialContacts` extraction path validates evidence presence, not person-uniqueness). A common name (e.g. "John Smith") must not be treated as a match without independent corroboration (shared former employer, matching role/title continuity, a source that explicitly links both companies) — this needs new, dedicated grounding logic before any of this is trustworthy, most acutely for signal type 4.

**Architecture fit — founder's working hypothesis confirmed by this recon, not contradicted:** existing-customer job/contact changes (types 1-3) belong in Core monitoring, as a refinement of the existing Leadership Change signal family (extend `context.contactName` to check every known contact, and add the new absence-detection check as a genuinely new recurring pattern) — reusing the account's own already-persisted contact identity, not a new data model. Prospect warm-path research (type 4) belongs later in the Pursue flow (once Find More Like Them's own Pursue/Target-Account destination exists — currently recon-only, not built), triggered only after a rep explicitly pursues a specific target, never run across every generated candidate — matching Find More Like Them's own stated economic-separation doctrine exactly.

**Possible future signal examples (illustrative, not committed copy):** "Your primary contact appears to have left Acme." / "Acme has a new VP of Marketing." / "Jane Smith, previously at Customer A, now appears to work at Target B."

**Trust requirement:** person/job-change identity must be strongly grounded before ever surfacing; a false employment match is unacceptable — see the grounding gap noted above.

**Before any build:** validate public-source detection quality and cost on a bounded real-contact fixture set (a handful of real, consented known-departure/known-move cases) before adding any recurring person-centric monitoring — this is a detection-quality question that needs real evidence, not an assumption, the same discipline already applied to Find More Like Them's own candidate-generation work.

**Do not build any of this now** — no monitoring queries, schemas, UI, or Prospecting behavior were changed by this recon.
